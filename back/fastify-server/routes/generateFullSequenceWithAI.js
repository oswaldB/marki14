// Route Fastify pour générer une séquence complète avec IA
// Migration depuis Parse.Cloud.define('generateFullSequenceWithAI')
...
export default async function (fastify) {
  ...
  // POST /api/generate-full-sequence - Génère une séquence complète avec IA
  fastify.post('/api/generate-full-sequence', async (request, reply) => {
    try {
      const { 
        sequenceId, 
        target, 
      ...
      const sequence = await db('sequences').where('objectId', sequenceId).first()
      ...
      await db('sequences').where('objectId', sequenceId).update({
  ...
  fastify.get('/api/test-generate-full-sequence', async (request, reply) => {
    return {
      message: 'Route generateFullSequenceWithAI migrée avec succès depuis Parse Cloud',
      originalFunction: 'Parse.Cloud.define("generateFullSequenceWithAI", ...)',
      newEndpoint: 'POST /api/generate-full-sequence',
      example: {
        method: 'POST',
        url: '/api/generate-full-sequence',
        body: {
      ...
      features: [
        'Génération complète de séquences de relances',
        'Support pour les particuliers et professionnels',
        'Gestion des multiples impayés',
        'Personnalisation du ton (professionnel, amiable, ferme, juridique)',
        'Seuil de transmission à huissier configurable',
        'Génération d\'actions avec délais progressifs',
        'Fallback automatique en cas d\'échec de l\'IA'
      ]
    }
  })
}
...
// Fonction pour obtenir un nom de payeur d'exemple
function getSamplePayeurName(target) {
  const names = {
    'particulier': 'M. Jean Dupont',
    'professionnel': 'Société ABC',
    'syndic': 'Syndic Immobilier',
    'locataire': 'Mme Marie Martin',
    'proprietaire': 'M. Pierre Durand'
  }
  return names[target] || 'Client'
}