...
document.addEventListener('alpine:init', () => {
  Alpine.data('impayesState', () => ({
    ...
    // Nouveaux filtres
    payerTypeFilter: '',     // Filtre par type de payeur (Propriétaire, Apporteur d'affaire, Autre)
    ...
    get impayesByPayeur() {
      console.log('Regroupement des factures par payeur avec regroupement par numéro de facture unique...');
      const grouped = {};
      const uniqueInvoices = new Map(); // Pour regrouper par numéro de facture unique (nfacture uniquement)
      
      // D'abord, regrouper toutes les factures par leur numéro de facture unique
    ...
    get sortedImpayesByPayeur() {
      ...
      groups.sort((a, b) => {
        ...
        switch (this.sortBy) {
          ...
          case 'date': // Date de la facture la plus récente
    ...
    /**
     * Ajouter une facture à un contact dans le regroupement
     * @param {Map} contacts - Map des contacts
     * @param {string} contactName - Nom du contact
     * @param {Object} invoice - Facture à ajouter
     */
    addInvoiceToContact(contacts, contactName, invoice) {
      ...
      // Mettre à jour le retard maximum
      const invoiceDelay = this.calculateDaysOverdue(invoice.datepiece);
     ...
     * @returns {Array} Contacts non triés
     */
    ...
    get impayesToFix() {
      console.log('Filtrer les factures à réparer...');
    ...
    async init() {
      try {
        ...
        // D'abord essayer de charger les données existantes depuis Parse
        // Cela évite une synchronisation coûteuse si les données sont déjà disponibles
    ...
    detectViewModeFromUrl() {
      ...
      // Trouver le mode de vue correspondant ou utiliser le mode par défaut
      this.viewMode = pathToViewMode[pathname] || 'byPayeur';
      console.log('Mode de vue défini à:', this.viewMode);
    },
    
    /**
     * Synchronise les données depuis PostgreSQL vers Parse
     */
    async syncImpayesFromPostgres() {
      console.log('Synchronisation des données depuis PostgreSQL...');
      ...
      try {
        ...
        // Après la synchronisation, récupérer les données mises à jour
        await this.fetchImpayes(); 
        
      } catch (error) {
        console.error('❌ Erreur lors de la synchronisation:', error);
        // Même en cas d'erreur de synchronisation, essayer de récupérer les données existantes
        await this.fetchImpayes();
      }
    },
    ...
    async fetchImpayes() {
      ...
      try {
        ...
        // Créer une requête pour les factures impayées - Utiliser "Impayes" avec un "s"
        ...
        // Limite à 99999 enregistrements comme spécifié
        query.limit(99999); 
    ...
    /**
     * Définit le mode d'affichage actuel
     * @param {string} mode - Le mode d'affichage à définir
     */
    setViewMode(mode) {
      console.log('Changement de mode d\'affichage:', mode);
      this.viewMode = mode; 
      
      // Logs de débogage pour vérifier l'état
      console.log('📊 État actuel après changement de mode:', {
    ...
    /**
     * Met à jour la requête de recherche avec debouncing
     */
    updateSearch() {
      console.log('Requête de recherche mise à jour:', this.searchQuery); 
      
      // Le filtrage réel est géré par la propriété calculée
    ...
    async performSearch() {
      ...
      try {
        ...
        // Simuler un délai de recherche (remplacer par une vraie recherche API si nécessaire)
    ...
    toggleSortDirection() {
      ...
      console.log('📊 Direction de tri basculée:', this.sortDirection);
    },
    ...
    setListSortBy(sortBy) {
      ...
      console.log('📊 Tri de la vue liste mis à jour:', {
    ...
    toggleListSortDirection() {
      ...
      console.log('📊 Direction de tri de la vue liste basculée:', this.listSortDirection);
    },
     ...
     */
    isMobileDevice() {
      // Vérifier la largeur de l'écran et l'agent utilisateur
    ...
    /**
     * Tronquer du texte à une longueur maximale
     * @param {string} text - Texte à tronquer
     * @param {number} maxLength - Longueur maximale (240 par défaut)
    ...
    formatAddress(invoice) {
      ...
      // Ajouter le numéro et type de voie
      if (invoice.numVoie) {
      ...
      // Ajouter le complément d'adresse
      ...
      // Ajouter le numéro de lot, étage, entrée, escalier, porte
    ...
    /**
     * Obtenir la date de la facture la plus récente pour un groupe
     * @param {Array} invoices - Liste de factures
     * @returns {number} Timestamp de la date la plus récente
     */
    ...
    /**
     * Obtenir les séquences uniques pour un groupe de factures
     * @param {Array} invoices - Liste de factures
     * @returns {Array} Liste de séquences uniques
     */
    ...
    /**
     * Vérifier les emails manquants et déclencher la synchronisation
     */
    async verifyMissingEmails() {
      console.log('🔍 Vérification des emails manquants...'); 
    ...
    exportMissingEmailsToCSV() {
      console.log('📥 Export des emails manquants au format CSV...'); 
      ...
      if (missingEmails.length === 0) {
        console.log('⚠️ Aucun email manquant trouvé');
        return null;
      }
     ...
     * @param {Object} contact - Objet contact avec les informations à copier
     */
  ...
  - Facture #${invoice.nfacture} (${invoice.resteapayer || 0}€, ${this.calculateDaysOverdue(invoice.datepiece)}j retard)`).join('')
      ...
      navigator.clipboard.writeText(textToCopy).then(() => {
        ...
        this.showTemporaryNotification(`Informations de ${contact.name} copiées !`);
      }).catch(err => {
