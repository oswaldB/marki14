...
process_webhook() {
    ...
    vibe -p "Ton role est de vérifier que l $implementation_file à destination des développeurs respecte bien tous les précepts des guides. Si non, tu corriges $implementation_file. Tu vérifies aussi si le recours à Fastify est vraiement nécessaire.Si non, tu corriges $implementation_file. Tu vérifies si le découpage des fichiers alpinjs en plusieurs modules est vraiment nécessaire.Si non, tu corriges $implementation_file." 
    sleep 10
    ...
    sleep 10
    vibe -p "Tu lis les guides/ et tu vérifies que $implementation_file est bien complètement développé. si oui tout est bon. Si non tu corriges." 
    sleep 10
    echo "test astro"
    cd front
